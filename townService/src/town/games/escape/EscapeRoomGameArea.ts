  ...
  private _stateUpdated(updatedState: GameInstance<EscapeRoomGameState>) {
    if (updatedState.state.status === 'OVER') {
      ...
      if (gameID && !this._history.find(eachResult => eachResult.gameID === gameID)) {
        const { playerOne, playerTwo } = updatedState.state;
        if (playerOne && playerTwo) {
          const playerOneName =
            this._occupants.find(eachPlayer => eachPlayer.id === playerOne)?.userName || playerOne;
          const playerTwoName =
            this._occupants.find(eachPlayer => eachPlayer.id === playerTwo)?.userName || playerTwo;
          this._history.push({
            gameID,
            scores: {
              [playerOneName]: updatedState.state.winner === playerOne ? 1 : 0,
              [playerTwoName]: updatedState.state.winner === playerTwo ? 1 : 0,
            },
          });
        }
      }
